...
namespace TexStyle.Areas.YarnDyeing.Controllers {
    ...
    public class ChemicalController : Controller {
        ...
        public ChemicalController(IUnitOfWork uow, IMapper mapper) {
        ...
        public async Task<IActionResult> Index() {
            return View((await _uow.ChemicalService.GetAll()).ToList());
        }
        //[HttpGet]
        ...
        //    return View(_uow.ChemicalService.GetBetweenDateRange(options.sd.Value, options.ed.Value));
        //}
        [HttpGet]
        public async Task<IActionResult> AddOrUpdate(long? id) {
            ChemicalViewModel vm = null;
            if (id.HasValue) {
                vm = _mapper.Map<ChemicalViewModel>(await _uow.ChemicalService.GetById(id.Value));
            }
            return PartialView(vm);
        }

        [HttpPost]
        public async Task<IActionResult> AddOrUpdate(long? id, ChemicalViewModel vm) {
            if (ModelState.IsValid) {
                try {
                    var m = _mapper.Map<Chemical>(vm);
                    if (!id.HasValue) {
                        // create
                        await _uow.ChemicalService.Create(m);
                        _tempData.MSG = "Successfully Created";
                    } else {
                        //update
                        await _uow.ChemicalService.Update(m);
        ...
        public async Task<IActionResult> SelectChemical(long id)

        {
            var chemical =await _uow.ChemicalService.GetById(id);
            chemical.Selected = true;

            await _uow.ChemicalService.Update(chemical);

            return PartialView(chemical);
        }


        public async Task<IActionResult> UnSelectChemical(long id)
        ...
        public async Task<IActionResult> Delete(long id) {
            try {
                var o =await _uow.ChemicalService.GetById(id);

                if (o == null)
                    return new StatusCodeResult(StatusCodes.Status404NotFound);

                await _uow.ChemicalService.Delete(o);
