...
namespace TexStyle.Areas.ProductionPlaningControl.Controllers {
    ...
    public class RecipeController : Controller {
        ...
        public RecipeController(IUnitOfWork uow, IMapper mapper, IOptions<CompanySettings> companySettings) {
        ...
        public async Task<IActionResult> Index([FromQuery] FilterOptions options) {
            ...
            var pendingTimes =await _uow.RecipeService.PendingDyeingTimeAddition(Convert.ToInt32(userid));
            if (pendingTimes.Count() > 0)
            {
                ViewBag.PendigMessage = "Machine Start,Unload and Soaping Drain Times are missing in some recipes. Please add missing times before creating new recipe";
            }
            var x =await _uow.RecipeService.RecipeIndexViewModelRepository(options.sd.Value, options.ed.Value);
          //  var vm = _mapper.Map<List<RecipeViewModel>>(x);

            return View(x);
        }
        [HttpGet]
        public async Task<IActionResult> GetFormateByTypeYarn()
        
        
        {
            var recipeFormatList =await _uow.RecipeFormatHeaderService.GetAllYarn();

          
                   return PartialView(recipeFormatList);


            //return PartialView("~/Views/Shared/_SelectList.cshtml", new SelectListItemViewModel
            //{
            //    Name = "ProcessTypeId",
            //    PlaceHolder = "Select RecipeType",
            //    UseDefault = true,
            //    SelectList = recipeFormatList

            //});
        }
        public async Task<IActionResult> GetFormateByTypeFabric()
        {
            var recipeFormatList =(await _uow.RecipeFormatHeaderService.GetAllFabric()).ToSelectList();
            return PartialView("~/Views/Shared/_SelectList.cshtml", new SelectListItemViewModel
            {
                Name = "ProcessTypeId",
                PlaceHolder = "Select Recipe Formate",
        ...
        public async Task<IActionResult> AddOrUpdate(long? id) {
            ...
            var recipeFormatList =(await _uow.RecipeFormatHeaderService.GetAll()).ToSelectList();            
            ...
            if (id.HasValue) {
                ...
                //var cs = await _uow.DyeChemicalTrService.GetByRecipeNo(id.Value);
                if (cs > 1)
                {
                    throw new Exception();
                }
        ...
        public async Task<IActionResult> AddOrUpdate([FromRoute]long? id, [FromForm]RecipeViewModel vm) {
            if (ModelState.IsValid) {
                try { if(vm.IsYarn==true)
                    vm.Id = null;
                    var m = _mapper.Map<Recipe>(vm);
                    if (vm.IsYarn == true)
                    {
                        m.IsYarn = true;
                    }
                    if (vm.IsFabric == true)
                    {
                        ...
                        m.UserId = Convert.ToInt32(User.Identity.GetUserId());
                    if (!id.HasValue) {
                        var item =(await _uow.RecipeService.GetAll()).Count();
                        int max = (int)(await _uow.RecipeService.GetAll(true)).Max(x => x.No);
                        m.No = item == 0 ? 1.0m : (max + 1.0m);
                        m.IsReprocessed = false;
                      
                        Recipe recipe = await _uow.RecipeService.Create(m);
                        RecipeDetailAddUpdate(Convert.ToInt64(m.RecipeFormatId), m.Id, true);
                        _tempData.MSG = "Successfully Created";
                    } else {
                        //update

                        m.Id = id.Value;
                        Recipe recipe = await _uow.RecipeService.GetById(m.Id);
                        if (recipe.RecipeFormatId != m.RecipeFormatId) {
                            RecipeDetailAddUpdate(Convert.ToInt64(m.RecipeFormatId), m.Id, false);
                        }
                        await _uow.RecipeService.Update(m);
                        _tempData.MSG = "Successfully Updated";
                    }
                    return RedirectToAction(nameof(Details), "Recipe", new { id = m.Id });
                }
                catch (Exception ex) {
                    throw ex;
        ...
        public async Task RecipeDetailAddUpdate(long recformatId, long recipeId, bool isadd) {
            ...
            if (recipeDetail != null) {
                ...
                await Task.WhenAll(recipeDetail.Select(async d =>
                {
        ...
        public async Task<IActionResult> AddLPS(long id) {
            ...
            var recipe = await _uow.RecipeService.GetById(id);
            ViewBag.Res = recipe;
            ViewBag.Reprocessed = recipe.IsReprocessed;
        ...
        public async Task<IActionResult> AddLPS(long id, RecipeLPSViewModel vm)  {
            ...
            Recipe r = await _uow.RecipeService.GetById(id);
            //var i =await _uow.RecipeLPSService.GetByLps(vm.LPSId.Value);
            var x =await _uow.PPCPlanningService.GetById(vm.LPSId.Value);

            var LpsAdded = (await _uow.RecipeLPSService.GelListByLps(vm.LPSId.Value)).ToList();

            if (LpsAdded.Count > 0) {

                List<RecipeLPS> fabricPrintingLps = new List<RecipeLPS>();
                List<RecipeLPS> garmentPrintingLps = new List<RecipeLPS>();
                List<RecipeLPS> garmentDyeing = new List<RecipeLPS>();
                List<RecipeLPS> dyeingLps = new List<RecipeLPS>();

                foreach (var item in LpsAdded)
                {
                    var r1 = await _uow.RecipeService.GetById(item.RecipeId);
                    if (r1.IsFabricPrinting)
                    {
                        fabricPrintingLps.Add(item);
                    }
                    else if (r1.IsGarmentPrinting)
                    {
                        garmentPrintingLps.Add(item);
                    }
                    else if (r1.IsGarmentDyeing)
                    {
                        garmentDyeing.Add(item);
                    }
                    else
                    {
                        dyeingLps.Add(item);
                ...
                if (r.IsFabricPrinting == true && fabricPrintingLps.Count > 0)
                {
                    ViewBag.ErrorMessage = "This LPS is alredy used in Fabric Printing!";
                    return View(vm);
                }
                else if (r.IsGarmentPrinting == true && garmentPrintingLps.Count > 0)
                {
                    ViewBag.ErrorMessage = "This LPS is alredy used in Garment Printing!";
                    return View(vm);
                }
                else if (r.IsGarmentDyeing == true && garmentDyeing.Count > 0)
                {
                    ViewBag.ErrorMessage = "This LPS is alredy used in Garment Dyeing!";
                    return View(vm);
                }
                else if (r.IsFabricPrinting == false && r.IsGarmentPrinting == false && dyeingLps.Count > 0)
                {
                    ViewBag.ErrorMessage = "This LPS is alredy used in Dyeing!";
                    return View(vm);
                }
            }
            ...
            if (r.IsGarmentPrinting == true)
            {
                r.Pcs = await _uow.RecipeLPSService.GetPcsByRecNo(id, r.IsReprocessed);
            }
        ...
        public async Task<IActionResult> CalculateWeight(long id) {
            ...
            await Task.WhenAll(detail.RecipeDetails.Select(async d =>
            {
                if (d.Gpl != 0)
                {
                    d.Weight = 0;

                    d.Weight = (detail.LiquorRate * detail.Weight * d.Gpl) / 1000;
                ...
                {
                    d.Weight = 0;
                    d.Weight = (detail.Weight * d.Percentage) / 100;

                }
                await _uow.RecipeDetailService.Update(d);
            }));
                ...
                try
                {
                   var res =await _uow.RecipeService.Create(r);
                    ...
                    if (r.IsReprocessed == true)
                    {
                        if (y.Count > 0)
                        {
                            await Task.WhenAll(y.Select(async lp =>
                            {
                    ...
                    await Task.WhenAll(r.RecipeDetails.Select(async d =>
                    {
                        d.RecipeId = r.Id;
                        await _uow.RecipeDetailService.Create(d);
                    }));                   
        ...
        public async Task<IActionResult> DyeingRecipe_D2Report(long id)
        {
            ...
            var reclps = (await _uow.RecipeService.Vw_RecipeLpsDetail(id)).ToList();
            ...
            if (d.Count()>0)
            {
                ...
                r.MachineStartTime = d.FirstOrDefault().MachineStartTime;
                r.MachineUnloadTime = d.FirstOrDefault().MachineUnloadTime;
                r.SoapingDrainTime = d.FirstOrDefault().SoapingDrainTime;
                r.IsGarmentPrinting = d.FirstOrDefault().IsGarmentPrinting;
                r.IsGarmentDyeing = d.FirstOrDefault().IsGarmentDyeing;
                r.Pcs = d.FirstOrDefault().Pcs;
                r.Vw_RecipeLpsDetail = reclps;
                ...
                d.ToList().ForEach(b =>
                {
                    if (b.RecipeStepId != null)
                    {
                        recipeStepId = b.RecipeStepId;
                    }
