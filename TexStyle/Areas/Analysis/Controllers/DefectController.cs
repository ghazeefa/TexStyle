...
namespace TexStyle.Areas.Analysis.Controllers
{
    ...
    public class DefectController : Controller
    {
        ...
        private string _ViewPath = $"/Areas/{AreaConstants.Analysis.Name}/Views/{nameof(Defect)}";

        private readonly TempDataViewModel _tempData;
        private readonly IDefectService _defectService;
        private readonly IMapper _mapper;
        public DefectController(IDefectService defectService, IMapper mapper)
        {
            AreaName = $"{AreaConstants.Analysis.NormalizedName} / {nameof(Defect)}";
        ...
        {
            return View(await _defectService.GetAll());
        }
        ...
        {
            DefectViewModel vm = null;
            if (id.HasValue)
            {
                vm = _mapper.Map<DefectViewModel>(await _defectService.GetById(id.Value));
            }
            return PartialView($"{_ViewPath}/{nameof(AddOrUpdate)}.cshtml", vm);
        }

        [HttpPost]
        public async Task<IActionResult> AddOrUpdate(long? id, DefectViewModel vm)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    var m = _mapper.Map<Defect>(vm);
                    if (!id.HasValue)
                    {
                        // create
                        await _defectService.Create(m);
                        _tempData.MSG = "Successfully Created";
                    }
                    else
                    {
                        //update
                        await _defectService.Update(m);
        ...
        public async Task<IActionResult> Delete(long? id, IFormCollection col)
        {
            try
            {
                if (id.HasValue)
                {
                    await _defectService.Delete(await _defectService.GetById(id.Value));
