...
{    [Area(AreaConstants.PRODUCTION_PLANING_CONTROL.Name)]
    public class SeasonController : Controller
    {
        ...
        private string _ViewPath = $"/Areas/{AreaConstants.PRODUCTION_PLANING_CONTROL.Name}/Views/{nameof(Season)}";
        ...
        {
            AreaName = $"{AreaConstants.PRODUCTION_PLANING_CONTROL.NormalizedName} / {nameof(Season)}";
        ...
        {
            return View((await _seasonService.GetAll()).ToList());
        }
        //[HttpGet]
        ...
        //    return View(_seasonService.GetBetweenDateRange(options.sd.Value, options.ed.Value));
        //}
        public async Task<IActionResult> AddOrUpdate(long? id)
        {
            SeasonViewModel vm = null;
            if (id.HasValue)
            {
                vm = _mapper.Map<SeasonViewModel>(await _seasonService.GetById(id.Value));
            }
            return PartialView($"{_ViewPath}/{nameof(AddOrUpdate)}.cshtml", vm);
        }

        [HttpPost]
        public async Task<IActionResult> AddOrUpdate(long? id, SeasonViewModel vm)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    var m = _mapper.Map<Season>(vm);
                    if (!id.HasValue)
                    {
                        // create
                        await _seasonService.Create(m);
                    }
                    else
                    {
                        //update
                        await _seasonService.Update(m);
                    }
        ...
        public async Task<IActionResult> Delete(long? id, IFormCollection col)
        {
            try
            {
                if (id.HasValue)
                {
                    await _seasonService.Delete(await _seasonService.GetById(id.Value));
