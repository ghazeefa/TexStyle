...
namespace TexStyle.Areas.Gate.Controllers
{
    ...
    public class GateIGPDyesChemicalDetailController : Controller
    {
        public IUnitOfWork _uow { get; set; }
        public IMapper _map { get; set; }
        private readonly TempDataViewModel _tempData;
        public GateIGPDyesChemicalDetailController(IUnitOfWork uow, IMapper map)
        {
            _uow = uow;
            _map = map;
        ...
        public async Task<IActionResult> AddOrUpdate(long? id, GateTrDetailViewModel v)
        {
            ...
            return RedirectToAction("Details", "GateIGPDyesChemical", new { id = vm.GateTrId });
        }
        [HttpPost]
