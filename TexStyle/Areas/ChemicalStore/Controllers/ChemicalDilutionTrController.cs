...
namespace TexStyle.Areas.ChemicalStore.Controllers {
    ...
    public class ChemicalDilutionTrController : Controller {
        private readonly IUnitOfWork _uow;
        private readonly IMapper _mapper;
        private readonly TempDataViewModel _tempData;
        public ChemicalDilutionTrController(IUnitOfWork uow, IMapper mapper) {
            _uow = uow;
            _mapper = mapper;
        ...
        public async Task<IActionResult> Index([FromQuery] FilterOptions options) {
            ...
            return View(await _uow.DyeChemicalTrService.GetBetweenDateRange(options.sd.Value, options.ed.Value , ChemicalTransactions.Dilution));
        }
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    if (!id.HasValue)
                    {
                        ...
                        return RedirectToAction("Details", "ChemicalDilutionTr", new { Id = m.Id });
                    }
                    else
                    {
                        //update
        ...
        public async Task<IActionResult> ChemicalDilutionDetailReport(long id)
        {
            try
            {
                ...
                ViewBag.reportStatus = "IN/OUT Dilution";
                return View(planin);
            }
            catch (Exception)
            {
