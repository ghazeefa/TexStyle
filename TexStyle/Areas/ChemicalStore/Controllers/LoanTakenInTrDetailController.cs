...
namespace TexStyle.Areas.ChemicalStore.Controllers {
    ...
    public class LoanTakenInTrDetailController : Controller {
        private readonly IUnitOfWork _uow;
        private readonly IMapper _mapper;
        private readonly TempDataViewModel _tempData;

        public LoanTakenInTrDetailController(IUnitOfWork uow, IMapper mapper) {
            _uow = uow;
            _mapper = mapper;
        ...
        public async Task<IActionResult> AddOrUpdate(long? id, long ParentId)
        {
            ...
            if (id.HasValue)
            {
                ...
                if (vm.ChemicalId.HasValue) chemicalList.Find(x => Convert.ToInt64(x.Value) == vm.ChemicalId).Selected = true;
                if (vm.DyeId.HasValue) dyeList.Find(x => Convert.ToInt64(x.Value) == vm.DyeId).Selected = true;
        ...
        public async Task<IActionResult> AddOrUpdate(long? id, TrDetailViewModel vm)
        {
            ...
            return RedirectToAction("Details", "LoanTakenInTr", new { Id = vm.DyeChemicalTrId });
        }
